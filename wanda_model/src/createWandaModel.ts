...
// Generated from ObjectSculptSpec target: Wanda
// Sculpt build pass: blockout
// This factory is intentionally pass-gated. Finish browser screenshot review before unlocking deeper passes.
export function createWandaModel(options: ProceduralModelOptions = {}): THREE.Group {
  ...
  const mesh_root_0 = new THREE.Group() as unknown as THREE.Mesh;
  mesh_root_0.name = "Wanda";

  const skinMat = new THREE.MeshStandardMaterial({ color: 0xffd1b3, roughness: 0.4 });
  const suitMat = new THREE.MeshStandardMaterial({ color: 0x5a1111, roughness: 0.7, side: THREE.DoubleSide });
  const darkMat = new THREE.MeshStandardMaterial({ color: 0x1a1a1a, roughness: 0.8 });
  const hairMat = new THREE.MeshStandardMaterial({ color: 0x6e2c1e, roughness: 0.6 });
  const crownMat = new THREE.MeshStandardMaterial({ color: 0x801010, roughness: 0.3, metalness: 0.2 });

  const torsoGeo = new THREE.CylinderGeometry(0.35, 0.3, 1.2, 16);
  const torso = new THREE.Mesh(torsoGeo, suitMat);
  torso.position.y = 0.6;
  torso.castShadow = true;
  mesh_root_0.add(torso);

  const headGeo = new THREE.SphereGeometry(0.3, 32, 32);
  const head = new THREE.Mesh(headGeo, skinMat);
  head.position.y = 1.45;
  head.castShadow = true;
  mesh_root_0.add(head);

  const crownGeo = new THREE.TorusGeometry(0.31, 0.04, 16, 32, Math.PI);
  const crown = new THREE.Mesh(crownGeo, crownMat);
  crown.position.set(0, 1.5, 0.05);
  crown.rotation.x = Math.PI / 8;
  crown.castShadow = true;
  mesh_root_0.add(crown);
  
  const hornGeo = new THREE.ConeGeometry(0.05, 0.2, 8);
  const hornL = new THREE.Mesh(hornGeo, crownMat);
  hornL.position.set(-0.25, 1.6, 0.1);
  hornL.rotation.z = Math.PI / 6;
  const hornR = hornL.clone();
  hornR.position.x = 0.25;
  hornR.rotation.z = -Math.PI / 6;
  mesh_root_0.add(hornL, hornR);

  const hairGeo = new THREE.SphereGeometry(0.33, 16, 16, 0, Math.PI * 2, 0, Math.PI * 0.6);
  const hair = new THREE.Mesh(hairGeo, hairMat);
  hair.position.y = 1.45;
  hair.rotation.x = -0.2;
  mesh_root_0.add(hair);

  const armGeo = new THREE.CylinderGeometry(0.1, 0.08, 1.1, 16);
  const armL = new THREE.Mesh(armGeo, skinMat);
  armL.position.set(-0.5, 0.6, 0);
  armL.rotation.z = Math.PI / 12;
  const gloveL = new THREE.Mesh(new THREE.CylinderGeometry(0.085, 0.07, 0.6, 16), darkMat);
  gloveL.position.y = -0.25;
  armL.add(gloveL);
  mesh_root_0.add(armL);
  
  const armR = armL.clone();
  armR.position.x = 0.5;
  armR.rotation.z = -Math.PI / 12;
  mesh_root_0.add(armR);

  const legGeo = new THREE.CylinderGeometry(0.15, 0.1, 1.2, 16);
  const legL = new THREE.Mesh(legGeo, darkMat);
  legL.position.set(-0.18, -0.6, 0);
  mesh_root_0.add(legL);
  
  const legR = legL.clone();
  legR.position.x = 0.18;
  mesh_root_0.add(legR);

  const capeGeo = new THREE.PlaneGeometry(0.8, 1.8, 4, 4);
  const cape = new THREE.Mesh(capeGeo, suitMat);
  cape.position.set(0, 0.4, -0.3);
  cape.rotation.x = -0.1;
  mesh_root_0.add(cape);

  mesh_root_0.userData.sculptComponent = {"id": "root", "name": "Wanda", "level": "macro", "role": "body", "importance": 1.0, "confidence": 0.5, "primitive": "box", "topologyClass": "assembled-solid", "topologyRationale": "Placeholder root blockout; reclassify per Workstream A's decision tree (grimoire/intake/surface_topology.md) once real geometry is authored.", "geometryDescriptor": {"topologyIntent": "low-poly blockout with bevel-ready edges", "edgeTreatment": {"type": "none", "bevelRadius": 0.0, "segments": 1}, "deformationStack": [], "uvStrategy": "generated procedural coordinates", "normalStrategy": "vertex normals from generated geometry"}, "parent": null, "attachment": null, "dimensions": {"width": 1.0, "height": 1.0, "depth": 1.0, "units": "relative", "confidence": 0.5}, "transform": {"position": [0, 0, 0], "rotation": [0, 0, 0], "scale": [1, 1, 1]}, "actionProfile": {"animationRole": "root", "pivot": {"mode": "center", "localPosition": [0, 0, 0], "axis": [0, 1, 0], "confidence": 0.5}, "transformChannels": {"translate": true, "rotate": true, "scale": true, "bend": false, "twist": false, "detach": false, "visibility": true, "materialState": true}, "sockets": [], "collider": {"type": "box", "offset": [0, 0, 0], "scale": [1, 1, 1], "isTrigger": false, "notes": "Replace with sphere/capsule/compound proxy when the object shape demands it."}, "constraints": [], "destruction": {"breakable": false, "fractureGroup": "root", "seamRefs": [], "detachableFragments": [], "breakImpulse": 0.0, "debrisMaterial": "base"}}, "material": "base", "materialLayers": ["base"], "deformations": [], "joints": [], "seams": [], "localFeatures": [], "surfaceDetail": {"macroRoughness": 0.0, "microRoughness": 0.0, "bumpAmplitude": 0.0, "normalPattern": "", "displacementPattern": "", "occlusionPattern": "", "edgeWearPattern": "", "notes": ""}, "evidenceRefs": ["full-object"], "details": [], "fidelityTier": "blockout"};
...
export function createWandaLookDevLights(
  mode: 'neutral' | 'grazing' | 'reference' = 'neutral',
): THREE.Group {
  const lights = new THREE.Group();
  lights.name = "Wanda look-dev lights";
...
export function createWandaEnvironment(renderer: THREE.WebGLRenderer): THREE.Texture {
...
export function frameWandaCamera(
...
export function createWandaPresentationComposer(
...
export function configureWandaRenderer(renderer: THREE.WebGLRenderer): void {
...
export function createWandaInspectControls(
  camera: THREE.Camera,
